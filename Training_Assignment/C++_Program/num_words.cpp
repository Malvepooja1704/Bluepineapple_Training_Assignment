...
string num_to_word(int num,string s){
	...
	if(num>19){
		str += ten[num/10] + one[num%10];   //check num is greater than 19 for tenth place string 
	}
	else{
		str += one[num];        //1 to 19 in word assign
	} 	
	
	if(num){
		str += s;           //add that string
	}
	return str;
}
...
string conv_num_to_word(long long int num){
	...
	result += num_to_word((num/100000000000000000),"shankh ");       //count the digit whether it is thousand, lakh, crore etc
	                                                                 //pass it to the num_to_word function 
